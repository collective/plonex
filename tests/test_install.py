...
from plonex.services.install import name_as_pep503
...
@contextmanager
def stub_virtualenv(install):
    """Create a minimal .venv stub to avoid real virtualenv creation in tests."""
    venv_bin = install.target / ".venv" / "bin"
    venv_bin.mkdir(parents=True, exist_ok=True)
    (venv_bin / "activate").touch()
    (venv_bin / "uv").touch()
    with mock.patch.object(install, "ensure_virtualenv"):
        yield
...
def temp_install(**kwargs):
    with temp_cwd():
        ...
        install = InstallService(**kwargs)
        with stub_virtualenv(install):
            with install:
                yield install
...
def mock_remote_constraints(mapping=None):
    ...
    with mock.patch("plonex.services.install.requests.get", side_effect=fake_get):
        yield


class TestInit(PloneXTestCase):

    default_requirements_file = dedent(
        """\
        # This file is generated by plonex

        ## Minimal requirements to run Plone via plonex
        supervisor
        ZEO
        rich
        Plone

        ## You can add more requirements in any file in the etc/requirements.d/ folder

        ## You can also add direct pip requirement specs in the plonex.yml file with the pip_requirements option"""  # noqa: E501
    )
                ...
                with mock_remote_constraints():
                    with install:
                        ...
                        self.assertEqual(
                            ...
                            self.default_requirements_file,
                        )
                        self.assertListEqual(
    ...
    def test_requirements_append_pip_requirements_from_yaml(self):
        with temp_cwd() as cwd:
            install = InstallService(dont_ask=True)
            (install.etc_folder / "plonex.yml").write_text(
                "pip_requirements:\n" "  - waitress\n" "  - black\n"
            )
            (install.requirements_d_folder / "010-local.txt").write_text("foo\n")
            ...
            with stub_virtualenv(install):
                with mock_remote_constraints():
                    with install:
                        ...
                        self.assertEqual(
                            requirements_line.pop(9),
                            f"-r {cwd / 'etc' / 'requirements.d' / '010-local.txt'}",
                        )
                        self.assertEqual(requirements_line.pop(11), "waitress")
                        self.assertEqual(requirements_line.pop(11), "black")
                        self.assertEqual(
                            "\n".join(requirements_line), self.default_requirements_file
                        )

    def test_requirements_and_constraints_merge_from_profiles(self):
        with temp_cwd() as cwd:
            install = InstallService(dont_ask=True)

            base_profile = cwd / "profiles" / "base"
            child_profile = cwd / "profiles" / "child"
            (base_profile / "etc" / "requirements.d").mkdir(parents=True)
            (base_profile / "etc" / "constraints.d").mkdir(parents=True)
            (child_profile / "etc" / "requirements.d").mkdir(parents=True)
            (child_profile / "etc" / "constraints.d").mkdir(parents=True)
            ...
            (base_profile / "etc" / "requirements.d" / "010-base.txt").write_text(
                "base-req\n"
            )
            (child_profile / "etc" / "requirements.d" / "010-child.txt").write_text(
                "child-req\n"
            )
            (install.requirements_d_folder / "010-local.txt").write_text("local-req\n")

            (base_profile / "etc" / "constraints.d" / "010-base.txt").write_text(
                "shared==1.0.0\nbasepkg==1.0.0\n"
            )
            (child_profile / "etc" / "constraints.d" / "010-child.txt").write_text(
                "shared==2.0.0\nchildpkg==2.0.0\n"
            )
            (install.constraints_d_folder / "010-local.txt").write_text(
                "shared==3.0.0\nlocalpkg==3.0.0\n"
            )
            ...
            with stub_virtualenv(install):
                with mock_remote_constraints():
                    with install:
                        ...
                        self.assertEqual(
                            requirements_line.pop(9),
                            f"-r {child_profile / 'etc' / 'requirements.d' / '010-child.txt'}",  # noqa: E501
                        )
                        self.assertEqual(
                            requirements_line.pop(9),
                            f"-r {base_profile / 'etc' / 'requirements.d' / '010-base.txt'}",  # noqa: E501
                        )
                        self.assertEqual(
                            "\n".join(requirements_line), self.default_requirements_file
                        )
    ...
    def test_ensure_virtualenv_prompts_for_python_when_needed(self):
        with temp_cwd() as cwd:
            ...
            with mock.patch.object(
                ...
                with mock.patch("plonex.services.install.Console") as MockConsole:
    ...
    def test_add_packages(self):
        with temp_install() as install:
            with mock.patch("plonex.services.install.datetime") as mock_datetime:
    ...
    def test_developed_packages_and_paths(self):
        with temp_cwd():
            ...
            with mock.patch(
                "plonex.services.install.RequirementsFile.from_file",
    ...
    def test_run_with_persist(self):
        with temp_cwd() as cwd:
            ...
            with (
                ...
                mock.patch("plonex.services.install.datetime") as mock_datetime,
            ):
        ...
        with temp_cwd() as cwd:
            ...
            with mock.patch.object(install.logger, "warning") as mock_warning:
                with (
                    ...
                    mock.patch("plonex.services.install.Console") as MockConsole,
                ):
                    with install:
                        install.requirements_txt = requirements_file
                        install.constrainst_txt = constraints_file
    ...
    def test_run_with_remote_constraint_include_does_not_report_missing(self):
        with temp_cwd() as cwd:
            ...
            with mock.patch.object(install.logger, "warning") as mock_warning:
                with mock.patch(
                    "plonex.services.install.requests.get", return_value=remote_response
                ):
    ...
    def test_update_gitman_sources_runs_when_configured(self):
        with temp_cwd() as cwd:
            ...
            with (
                ...
                mock.patch.object(InstallService, "make_constraints_txt"),
                mock.patch.object(install, "run_command"),
                mock.patch("plonex.services.install.SourcesService") as MockGitman,
